<?php
    ...
    $stmt = $conexion->prepare("select c.nombre_categoria from activo a inner join categoria c on a.categoria_id = c.id_categoria where a.codigo = :codigoActivo;");
    ...
    if ($categoria == $nombreCategoriaInmubeles) {
        ...
        $stmt = $conexion->prepare("select er.persona_id from entrega_recepcion er inner join activo a on er.activo_id = a.id_activo where a.codigo = :codigoActivo;");
        ...
        $stmt = $conexion->prepare("select p.nombre_persona, p.cedula, c.nombre_cargo from persona p inner join cargo c on p.cargo_id = c.id_cargo where p.id_persona = :idPersona;");
